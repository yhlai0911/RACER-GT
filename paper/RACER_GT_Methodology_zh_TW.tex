...
\hypersetup{
  ...
  citecolor=black,
  urlcolor=blue,
  pdftitle={RACER-GT 方法論與 Python 套件技術報告},
  pdfauthor={Yi-Hao Lai and RACER-GT contributors}
}
...
\end{equation}
為反映事件日可能具有較大跨 pull dispersion，0.1.0 版再以當日 absolute deviation median 相對於全期間 median 進行 bounded local multiplier。這是 pragmatic heteroskedastic adjustment，而非完整 Bayesian posterior interval；正式研究可另以 block bootstrap 重新執行全 pipeline。
...
RACER-GT 0.1.0 採 ingestion-first architecture：統計套件不內建未經官方保證的網站 scraping。網站端點、rate limits、認證與服務條款可能變動，將 retrieval client 與估計器解耦，可使研究者在官方 API、手動 CSV export 或機構核准的 collector 間替換，而不改變統計 protocol。Collector 只需輸出固定 long-format schema。

核心模組如\cref{tab:modules}。

\begin{longtable}{p{3.5cm}p{10.5cm}}
\caption{RACER-GT 0.1.0 模組}\label{tab:modules}\\
\toprule
模組 & 功能\\
\midrule
\endfirsthead
...
第二，零值可能混合真實零搜尋、privacy threshold、sampling 與 rounding。RACER-GT 0.1.0 將零值保留並分離 detection reliability，但尚未內建 interval-censored latent count likelihood。對高 zero-share term，降為週頻、改用較廣 Topic 或 anchor-bank calibration，通常比強制製造日頻值更合理。
...
第四，benchmark 後的 0.1.0 信賴區間主要保留 cross-pull measurement uncertainty，尚未完整傳播 benchmark noise、scale calibration uncertainty 與 tuning-parameter uncertainty。最嚴謹方式是 nested moving-block bootstrap：每次重抽 historical blocks、重新估 edges、scales、duplicates、covariance、weights 與 benchmark，最後再重估財金模型。
