...
\texttt{CalibrationResult.diagnostics} reports
...
converter is \texttt{examples/ingest\_trends\_csv.py} and the analysis is
\texttt{examples/run\_real\_data\_calibration.py}.
...
least-squares fit at whatever weight \texttt{edge\_variance\_floor} and
\texttt{max\_edge\_weight} permit --- numerical safety constants, not a
statistical argument. \texttt{simulate\_racergt\_data} multiplies every chunk by
...
against 5 in 26 real ones. (Setting \texttt{chunk\_noise\_sd} to $0$ would let the
...
\texttt{n\_zero\_dispersion\_edges}, \texttt{n\_informative\_edges} and
\texttt{n\_scale\_groups} to \texttt{CalibrationResult.diagnostics}. They
...
Largest residual mis-scaling & $-0.032\%$ (all $|t|<0.6$)\\
\bottomrule
\end{tabular}
\end{table}
...
overlaps shows drift at $p<0.05$, against $1.05$ expected by chance. After
calibration no chunk deviates significantly from the reconstructed series, the
largest being $-0.032\%$. Real data does not refute the proportionality model.
...
simulator describes reality. \texttt{SimulationSettings.chunk\_noise\_sd}
...
\centering
\small
\caption{Sweeping \texttt{chunk\_noise\_sd} against the observed overlap
...
\subsubsection{One year of chunks has no power against F1, and that is computable}
...
largest absolute difference of $0.195$ points on the 0--100 index scale. F1's
signature is likewise absent --- the scale discrepancy does not grow along the
chain (last three positions against first three, ratio $1.03$; Spearman
$\rho=-0.024$, $p=0.955$).

This is \textbf{not evidence against F1}, because the design cannot accumulate
anything, and that can be computed rather than asserted. F1 operates by
accumulation, the terminal error variance of a chain is the sum of the variances
of the joins it walks, and a zero-dispersion join contributes nothing. Only 3 of
the 7 joins here are informative; their median standard error is $0.002037$, so
the terminal accumulated standard deviation is $0.0027$ log points, or
$\mathbf{0.270\%}$. Inverting that: reaching $1\%$ needs about 24 informative
joins, $5\%$ needs 574, and $10\%$ needs 2{,}189. Those three are
\emph{lower} bounds: the median standard error used is taken over all 21
informative overlaps, short ones included, whereas the adjacent joins of a
30-day-step design carry 150-day overlaps at a standard error near $0.0014$,
which would put $1\%$ at about 50 joins. The estimate is deliberately the one
that favours F1.

\begin{remark}[F1 is real and immaterial at any feasible scale]\label{rem:f1scale}
At the overlap lengths and dispersions actually observed, sequential stitching
would need roughly 574 informative joins before accumulated scale error reached
$5\%$. At a 30-day step that is far more than the years Google Trends has
existed. The $4.75$-fold growth visible in \cref{sec:calexp} appears because the
simulator overstates chunk noise by about a factor of 4.5.

The correct statement about F1 is therefore that the mechanism is real and
measurable, but that at realistic noise levels its accumulation is immaterial in
any feasible Google Trends design. The reason to prefer global graph calibration
is not accuracy but the diagnostics in \cref{tab:realdata} that a spanning path
structurally cannot provide: connectivity, condition number, and cycle closure.
This agrees with \cref{rem:pattern} and sharpens it: the first three results were
mechanisms that work with second-order effects, and this one is a mechanism that
works whose effect has a computable upper bound.
\end{remark}
