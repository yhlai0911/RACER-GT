...
unbiasedness and minimum-variance results under them. In twenty controlled
Monte Carlo replications, \RACER{} attains a mean RMSE of $3.5265$ against a
known latent truth, compared with $7.2305$ for a single pull, $4.1710$ for the
cross-pull median, and $3.7253$ for the simple cross-pull mean. These results
support the claim that the framework reduces reconstruction error under the
stated data-generating process; they are not a universal optimality guarantee
across keywords, regions, or GT versions.
...
Search data have become a standard empirical input in finance. \citet{DaEG2011}
...
are retained in the final series and analyzed separately through detection
reliability. \Cref{as:connected} fails if the chunk design leaves a gap; the
...
\label{eq:benchmark}
\end{equation}
where $Q$ combines fidelity to the daily consensus with a smoothness penalty on
the correction path, and $W$ is the benchmark precision. This is the
...
\begin{table}[H]
\centering
\caption{Twenty controlled replications. Lower RMSE is better.}
\label{tab:mc}
\begin{tabular}{lrrrr}
\toprule
...
\RACER{}           & \textbf{3.5265} & \textbf{0.9927} & 0.9745 & 0.9526\\
\bottomrule
\end{tabular}
\end{table}

Under this data-generating process, \RACER{} reduces mean RMSE by approximately
$51.2\%$ relative to a single pull and by $5.3\%$ relative to the simple mean.

\begin{remark}[Reporting the losses as well as the wins]
\RACER{} does not dominate the simple mean on every statistic in
\cref{tab:mc}: mean innovation correlation is marginally lower
($0.9745$ versus $0.9747$) and peak recall is identical. We report this because
the honest summary is that the gain concentrates in level accuracy, where
scale-error correction and dependence-aware weighting have most to contribute,
and is negligible for differenced quantities under this particular DGP. A
practitioner whose model uses only innovations should weigh that accordingly.
\end{remark}
