...
Under variance heterogeneity the picture is different. In a simplified controlled
process --- one latent series, nine pulls with lognormal retrieval noise whose
standard deviations form a geometric ladder, thirty replications, no chunking or
design facets --- the covariance-adjusted consensus wins every replication. At a
true variance ratio of $122$ the mean RMSE is $2.88$ against $5.59$ for a simple
mean; at $12144$ it is $3.86$ against $17.75$. The homogeneous case reproduces the
null result, with $2.040$ against $2.036$ and wins in $27\%$ of replications.

The practical recommendation of \cref{sec:monte-carlo} therefore needs its
condition attached: covariance adjustment brings no detectable improvement
\emph{when the pulls are homogeneous in variance}, which is what that design makes
them. Whether real retrievals differ in variance is an empirical question this
paper does not settle; it is the same repeated-measurement structure whose
existence \cref{sec:design} is built to test, and it has not yet been measured on
real data. Both statements should be read together: nothing here shows the
weighting helps on Trends data, and nothing shows it cannot.
