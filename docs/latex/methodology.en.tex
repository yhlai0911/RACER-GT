...
$p=0.152$). We report that negative result rather than the favourable
comparison, and explain it: dependence across pulls is highly homogeneous under
this data-generating process, whereas the theoretical advantage of GLS over
equal weights requires dependence strength to differ across pulls. None of these
figures is a universal optimality guarantee across keywords, regions, or GT
versions.
...
\begin{remark}[Why the covariance adjustment is retained]
The second conclusion is a negative result, and it bounds the claim rather than
refuting the method. Dependence in this DGP is deliberately homogeneous: every
pull shares its collection-day noise with fixed weight $0.5$, so the residual
covariance matrix is close to equicorrelated, and the minimum-variance weights
of an equicorrelated matrix are equal weights. The advantage of GLS over equal
weighting requires dependence strength to \emph{differ} across pulls, as it
would when some pulls are served from one cache and others are not. Detecting
that requires a factorial simulation varying dependence heterogeneity, which
this version does not provide.

The simple mean also cannot produce what \RACER{} reports alongside its series:
spectral and Kish effective pull counts, the near-duplicate dependence
structure, per-day conditional standard errors, and measurement uncertainty in a
form the downstream EIV correction can consume. Under this DGP those diagnostics
do not translate into lower RMSE, but they are what answers the question of
whether a batch is usable at all, which RMSE cannot answer.
\end{remark}
