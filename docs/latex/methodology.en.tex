...
conditional standard errors (read with \cref{sec:dailyse}), and propagatable
measurement uncertainty are information that RMSE neither expresses nor replaces.
\end{remark}
...
not yet seen. \Cref{sec:resolution} shows when $\bar{w}$ actually binds, which is
not on data of the kind analysed here, and why that is not a reason to remove it.
\end{remark}
...
\subsection{What the covariance can resolve}\label{sec:resolution}

$\hat{\Sig}$ is estimated from residuals taken about the daily central tendency of
the pulls themselves, so the subtrahend is correlated with what it is subtracted
from. The mathematical appendix shows that this caps how different the estimated
variances can be: with a mean subtrahend, as one pull becomes arbitrarily precise
the largest variance ratio $\hat{\Sig}$ can report tends to $m-1/(m-1)$,
independently of the true ratio. Simulation at a true variance ratio of
$4\times10^{8}$ matches to within $0.6\%$ for $m$ from three to ten. The median
the implementation uses is better by 3 to 50 percent and converges to the same
bound.

The bound is stated for one configuration --- a single arbitrarily precise pull
against an otherwise homogeneous set --- and it is not a universal ceiling. With
nine pulls whose standard deviations form a geometric ladder, a true variance
ratio of $122$ is reported as $45$, well above $m-1/(m-1)=8.9$. What the
proposition establishes is that centring on the sample costs resolution, and how
much in the case that can be solved exactly. Its sharpest instance is $m=2$, where
the bound is one identically: the residuals satisfy $r_1=-r_2$ pointwise, so no
covariance estimate can distinguish two retrievals however different their true
precisions are. Two-retrieval designs cannot weight by estimated precision, and
that holds for any data, not just Trends.

It would be tempting to read the null result of \cref{sec:monte-carlo} through
this bound, and it would be wrong. In the data-generating process used there every
pull carries the same error structure, so the true variance ratio is one and the
minimum-variance weights are equal weights: there is nothing for covariance
adjustment to exploit, and the bound never binds. The dependence-heterogeneity
experiment of \cref{sec:depexp} varies the correlation between pulls rather than
their variances, so it does not test this either.

Under variance heterogeneity the picture reverses. \Cref{tab:varhet} repeats the
Monte Carlo of \cref{sec:monte-carlo} --- same estimator, same seeds, same design,
same 20 replications --- varying only the spread of retrieval-noise standard
deviations across pulls, spread geometrically with geometric mean one so the noise
budget is redistributed rather than raised. At a variance ratio of 25 the
covariance-adjusted consensus wins all 20 replications, and the margin grows
monotonically with the spread.

\begin{table}[H]
\centering
\small
\caption{Covariance adjustment under variance heterogeneity, 20 replications per
row. Advantage is simple-mean RMSE minus \RACER{} RMSE, so positive favours
\RACER. The first row is the design of \cref{tab:mc} and reproduces its numbers.
The last row is a control: it restores homogeneous variances and introduces
correlation heterogeneity instead, where the adjustment is known to lose. All
rows are simulated; no row is evidence about real retrievals.}
\label{tab:varhet}
\begin{tabular}{lrrrrrl}
\toprule
Scenario & Var.\ ratio & \RACER & Simple mean & Advantage & $p$ & Wins\\
\midrule
Homogeneous (default) & $1$ & $3.7421$ & $3.7253$ & $-0.0167$ & $0.152$ & 7/20\\
Variance ladder 5 & $25$ & $3.4558$ & $4.3678$ & $0.9120$ & $<10^{-14}$ & 20/20\\
Variance ladder 10 & $100$ & $3.2853$ & $5.1407$ & $1.8554$ & $<10^{-15}$ & 20/20\\
Variance ladder 20 & $400$ & $3.1607$ & $6.3333$ & $3.1725$ & $<10^{-16}$ & 20/20\\
\midrule
\multicolumn{7}{l}{\emph{Control: can this comparison show \RACER{} losing?}}\\
Correlation only & $1$ & $3.9029$ & $3.8142$ & $-0.0887$ & $0.00049$ & 4/20\\
\bottomrule
\end{tabular}
\end{table}

The control row is not a secondary result. A comparison that cannot show the
covariance-adjusted estimator losing is not evidence that it wins, so the same
procedure is run on a design where it is known to lose, and it reports a
significant loss. The advantage above is therefore a property of the scenario and
not of the scoring.

One assumption in \cref{tab:varhet} runs in the direction that flatters the
estimator and so is measured rather than argued. Trends normalizes each window to
its own maximum, which makes retrieval error proportional, so the disturbance is
multiplicative; a multiplicative error is larger in absolute terms on high days,
and it is not obvious \emph{a priori} that this does not do the work. Repeating
the ladder with the same disturbance on the level scale gives an advantage of
$0.8534$ at a variance ratio of 25 against $0.9120$ multiplicative, and $1.7374$
at 100 against $1.8554$, in both cases winning all 20 replications. The
level-scale control is uniformly about six percent smaller and identical in sign
and significance, so the result is not an artefact of the error scale. Above a
ladder of about five the level-scale draw crosses zero; the clip that follows is a
nonlinearity the multiplicative case does not have, so the simulator warns and the
count of affected replications is reported. The ratio-25 comparison clips in none.

The practical recommendation of \cref{sec:monte-carlo} accordingly needs its
condition attached rather than withdrawn: covariance adjustment brings no
detectable improvement \emph{when the pulls are homogeneous in variance}, which is
what that design makes them, and it brings a large one when they are not. Whether
real retrievals differ in variance is an empirical question this paper does not
settle. Every row of \cref{tab:varhet} is simulated; the repeated-measurement
structure that would answer it on real data is what the acquisition design of
\cref{sec:design} exists to produce, and it has not yet been measured. Both halves
should be read together: nothing here shows the weighting helps on Trends data,
and nothing shows it cannot.

The reading of the constraints needs a correction of its own. Because the ratio
saturates on homogeneous designs,
the maximum weight stays near $0.35$ and $\bar{w}=0.50$ does not bind on any
configuration we have constructed. It would be wrong to conclude the cap guards
against nothing. Estimating $\hat{\Sig}$ from residuals about the current weighted
consensus and iterating --- the obvious way to escape the bound --- is positive
feedback: whichever pull gains weight has its residual shrunk, which raises its
apparent precision, which raises its weight again. The maximum weight reaches one
by the fourth pass and the reported variance ratio reaches $10^{11}$ by the fifth,
monotonically. Weight concentrating on a single pull is precisely what appears the
moment $\hat{\Sig}$ can discriminate, and in a sharper form than the remark above
anticipates. The cap is not slack because it is unnecessary; it is slack because
the quantity it guards has been suppressed upstream.

Where resolution does need to be recovered, the remaining route is an external
reference. Subtracting a series independent of every pull gives
$\Var(r_j)=\sigma_j^2+v$, whose ratio is unbounded as $v\to 0$, but the reference
must be more precise than the most precise pull: at $v$ corresponding to a
standard deviation of $1.0$ it recovers $0.31\%$ of a true ratio of $1600$, which
is what the median already delivers. Daily Google Trends offers no such series.
The frequency benchmark of \cref{sec:benchmark} is lower frequency by
construction, and the design-based estimator of \cref{sec:designest} is another
combination of the same pulls. What would supply it is an independent estimate of
per-pull variance from the repeated-measurement structure itself, which is what
the acquisition design of \cref{sec:design} exists to produce.

\subsection{Per-day standard errors, and what they are not}\label{sec:dailyse}

Each consensus series is published with a per-day standard error, and that
quantity is what the downstream correction of \cref{sec:eiv} consumes. Its exact
construction is given in the mathematical appendix; what matters here is the gap
between what it is and what it is easily read as.

It is a conditional \emph{scale} on cross-pull dispersion: one global
$\sqrt{\wvec^{\top}\hat{\Sig}\wvec}$ multiplied by the day's median absolute
deviation relative to a typical day, with that multiplier confined to
$[0.25,\,4]$. It is not a variance estimated separately for each day, and three
independent features of the construction push it downward --- the upper bound on
the multiplier, the fact that renormalizing weights on days with missing pulls
never enters the variance, and the shrinkage inherited from taking residuals
about the pulls' own daily median.

The direction is worth stating plainly, because it runs opposite to the
$N_{\text{spec}}$ bias just described. That one is conservative: it understates
information and errs toward rejecting a batch. These three err the other way,
making the series look more precise than the design supports.

A second per-day error is available and is now reported beside it. The calibration
stage of \cref{sec:calibration} produces, for every pull and every date, a standard
error built from the disagreement among the chunks covering that date, converted to
the level scale by the delta method. It is genuinely per-day and it varies with the
level: a date whose index value is small has a larger relative error, which is what
integer rounding to the 0--100 scale implies. On the real series of
\cref{sec:realdata} the ratio of this error to the value tracks the rounding
prediction $1/(\sqrt{12}\,v)$ to within 3 to 12 percent across value ranges.

The two are reported side by side and not summed. $\hat{\Sig}$ is estimated from
residuals that already contain calibration error, so adding them would double
count. What the calibration error provides is an independent lower bound: it comes
from within-day chunk disagreement, a derivation sharing nothing with the residual
covariance, so dates where it exceeds the consensus standard error are dates on
which the residual-based estimate is too small. The share of such dates is
reported. Propagation through the consensus weights assumes the calibration errors
are independent across pulls; \cref{sec:realdata} shows chunks within one
collection day are rescalings of a single served sample, and whether that extends
across pulls is not known, so the assumption travels with the result as a flag.

The division of labour is that the two quantities measure different things. The
calibration error is within one acquisition and is dominated by rounding and chunk
disagreement; the consensus standard error is across acquisitions and carries
whatever sampling variation repeated retrieval exposes. Only the first is available
without repeated collection.

This does not retract the claim that the framework supplies per-day uncertainty
where the alternatives supply none; that claim stands, and it is the substance of
\cref{sec:eiv}. What it limits is the weight that can be put on the
\emph{magnitude}. An error-corrected estimator given a uniformly understated
$\Omega$ under-corrects while every diagnostic it reports remains internally
consistent, so the output still reads as corrected, and separating that state
from a successful correction requires the true $\Omega$ --- which is exactly what
is unavailable. Results that turn on how large the measurement error is, rather
than on the fact that it exists, should therefore be read as conditional on a
lower bound.
...
\Cref{sec:resolution} attaches the condition under which that holds: this design
gives every pull the same error structure, so the minimum-variance weights are
equal weights by construction. Where the pulls differ in variance the same
estimator wins every replication by a wide margin.
...
EIV correction can consume --- subject, on the magnitude of that uncertainty, to
\cref{sec:dailyse}. RMSE cannot answer whether a batch is usable at all,
...
\subsection{What kind of measurement this applies to}\label{sec:appliesto}

Nothing above is specific to Google Trends. What the framework requires is that
the measurement be \emph{platform-mediated}: produced by an intermediary the
analyst does not control, which can change, and which does not publish the detail
of what it did. Four questions decide whether a series qualifies, and they are
separate questions rather than one.

\begin{enumerate}
\item Is the value a \emph{sample}, so that retrieving twice can differ?
\item Is there normalization \emph{within a request or sample}, and can its base
move?
\item Can the source change \emph{retrospectively}, so that one historical date
takes different values at different retrieval dates?
\item When the construction method changes, is history \emph{recomputed}?
\end{enumerate}

\begin{table}[H]
\centering
\small
\caption{Which series the framework addresses. \textbf{Only the first row has been
tested here}; the others are placed by structural correspondence, not by
measurement, and none of them is evidence for the framework.}
\label{tab:appliesto}
\begin{tabularx}{\textwidth}{Xcccc c}
\toprule
Series & Sampled & Normalized & Retro.\ change & Recomputed & Applies\\
\midrule
Google Trends (\textbf{measured}) & yes & per request & yes & yes & yes\\
Baidu Index, Naver DataLab & yes & per request & yes & yes & yes\\
Policy-uncertainty indices from news counts & no & to a base & \textbf{no}$^\dagger$ & yes & partly\\
Mobility indices vs.\ a base period & no & to a base & yes & yes & partly\\
Vendor sentiment and foot-traffic panels & panel & varies & yes & yes & partly\\
\midrule
\multicolumn{6}{l}{\emph{Outside the framework, and worth saying so}}\\
Composite sentiment gauges from market inputs & no & to $[0,100]$ & no & yes & no\\
Volatility indices with a published formula & no & no & no & yes & no\\
\bottomrule
\end{tabularx}
\raggedright\footnotesize
$^\dagger$Measured, not assumed. See the text below.
\end{table}

The two excluded rows matter more than the included ones. A composite gauge whose
inputs are observable market quantities --- momentum, breadth, option ratios,
credit spreads --- is not sampled and does not change retrospectively; where it is
hard to work with, the difficulty is undisclosed construction, which is a
reproducibility problem needing different tools. A volatility index with a
published formula can be recomputed by anyone from option prices. Claiming this
framework for them would be refuted immediately, and excluding them is what gives
the inclusion criteria content.

News-count uncertainty indices are the closest structural analogue, and the
correspondence is item by item: the article total in the denominator plays the
role of the window maximum, changes in which outlets a corpus covers play the role
of changes in Google's sampling, and the standardization base plays the role of
the baseline period. An earlier draft of this section marked their retrospective
change as likely on that basis. It was tested rather than left as an inference,
using an archived copy of a publisher's own file as a second retrieval date, and
the inference was wrong.

Over the 416 months both vintages cover, excluding the archived vintage's
unsettled tail, exactly one month differs by more than $10^{-6}$ in relative terms
--- the sixth month before the archived retrieval, which is a normal settling lag.
Every other difference lies below the single-precision floor at which the archived
file stores its numbers, so it is storage format rather than revision. Published
history is stable, and the framework's retrospective-change criterion does not
apply.

What did change is which series the file presents. In the archived vintage the
first sheet is named for the main index and its first numeric column is the
three-component measure; in the current one that sheet is named Legacy and the
first numeric column of the leading sheet is the news-based measure. The coverage
also extends backwards by more than eighty years. A researcher who opened the file
and took the first numeric column would have obtained different series at the two
dates, from the same publisher under the same filename. That is a reproducibility
problem, and a serious one, but it is a change of identity rather than of
measurement, so it is outside what this framework treats. Recording it here is the
point of testing rather than asserting: the analogy held on two criteria and
failed on the one it was invoked for.

\subsection{Relation to real-time data analysis}\label{sec:realtimedata}

The idea that a retrieval date is part of a dataset is not new to economics. The
real-time literature \citep{CroushoreStark2001} established that a macroeconomic
series is a sequence of vintages rather than a single object, that the same
quarter takes different values in different vintages, and that using final revised
values imparts look-ahead bias to studies of what was knowable at the time.

The framework here is the extension of that idea to a case where its central
assumption fails.

\begin{table}[H]
\centering
\small
\caption{Why vintage bookkeeping is not sufficient for platform-mediated series.}
\label{tab:vintage}
\begin{tabularx}{\textwidth}{lXX}
\toprule
 & Real-time macro data & Platform-mediated measurement\\
\midrule
Nature of change & Revision: deterministic update & Sampling: stochastic\\
Is there a final value & Yes; revisions converge & No\\
Correct practice & Use the vintage available then & Vintage is not enough: two
retrievals \emph{within} a vintage differ\\
Source of uncertainty & Estimable from revision history & Requires repeated
retrieval by design\\
\bottomrule
\end{tabularx}
\end{table}

Recording the retrieval date, which is what a vintage does, is necessary and not
sufficient here. Because there is no value the series converges to, the
uncertainty cannot be read off a revision history, and the only way to quantify it
is to retrieve repeatedly under a design fixed in advance. That is the whole
content of \cref{sec:design}, and it is why this paper is a repeated-measurement
protocol rather than an archiving convention.
