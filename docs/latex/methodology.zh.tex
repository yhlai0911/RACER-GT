...
\centering
\small
\caption{以\cref{eq:reff} 事前計算的校準誤差。$R_{\mathrm{eff}}$ 以 $\sigma^2$ 為單位。}
\label{tab:designreff}
\begin{tabular}{lrrrr}
\toprule
設計（歷史跨度、視窗／步進） & chunks & 降幅 & 逐段 & 全域圖\\
\midrule
1 年，180／30（本文實測） & 8 & $10.5\times$ & $0.40\%$ & $0.12\%$\\
5 年，180／30 & 56 & $18.2\times$ & $1.15\%$ & $0.27\%$\\
15 年，180／30 & 178 & $20.1\times$ & $2.06\%$ & $0.46\%$\\
15 年，180／15（\cref{tab:recommended} 建議） & 355 & $140.5\times$ & $\mathbf{2.79\%}$ & $\mathbf{0.23\%}$\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{remark}[F1 在本文自己建議的 protocol 下是實質的]
\label{rem:f1scale}
\cref{tab:designreff} 顯示，尺度誤差隨歷史跨度成長：15 年、180 日視窗、15 日步進——即\cref{tab:recommended} 所建議的規格——逐段 stitching 的終端尺度誤差為 $\mathbf{2.79\%}$，全域圖為 $\mathbf{0.23\%}$。對一條橫跨 15 年的注意力指數而言，$2.79\%$ 的端點尺度偏誤是一個緩慢漂移，形態上與趨勢難以區分，正是 GT 文獻反覆警告的 artificial low-frequency drift。

因此本文對 F1 的正確陳述是：\textbf{機制為真、可被證明（\cref{prop:reff}）、且在建議規模下具實質意義}；本文一年份的實測之所以看不到它，是因為該設計的絕對誤差上限只有 $0.40\%$。\cref{sec:calexp} 與\cref{sec:montecarlo} 的 null results 並非 F1 的反證，而是\cref{prop:reff} 的直接預測：在短鏈上有效電阻差距的絕對值必然微小。
