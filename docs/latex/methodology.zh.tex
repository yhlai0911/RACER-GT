...
在變異數異質下情況完全不同。以一個簡化的受控過程檢驗（單一潛在數列、9 個 pull、對數常態取得噪音且標準差呈幾何級數、30 次重複、無 chunk 結構與設計 facets），共變異數調整共識**每一次重複都勝出**：真實變異數比 $122$ 時平均 RMSE 為 $2.88$，simple mean 為 $5.59$；比值 $12144$ 時為 $3.86$ 對 $17.75$。同質情形則重現 null result，$2.040$ 對 $2.036$，30 次中僅勝 8 次。

因此 \cref{sec:montecarlo} 的實務建議必須附上條件：共變異數調整在 **pull 的變異數同質時**不帶來可偵測的改善，而那正是該設計所造成的狀態。真實取得之間是否存在變異數差異，是本文未解決的實證問題；它所需要的正是 \cref{sec:design-zh} 的取得設計要產生的重複量測結構，且尚未在真實資料上量測。兩句話必須一起讀：本文沒有證據顯示加權在 GT 資料上有幫助，也沒有證據顯示它不能。
