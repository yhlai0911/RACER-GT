...
上一小節的所有證據都出自本套件自己的模擬器，而模擬器是依估計量所假設的比例模型抽樣的。這是循環論證：它能顯示估計量解出了我們交給它的問題，卻不能顯示那個問題就是真實的問題。本節把校準階段套用在真實下載的資料上：關鍵字 \texttt{bitcoin}、地區美國、日頻、2024 全年，180 天視窗、30 天步進共 8 個 chunk，1{,}440 筆觀測。轉換器為 \texttt{examples/ingest\_trends\_csv.py}，分析為 \texttt{examples/run\_real\_data\_calibration.py}。
...
這樣的 edge 不帶任何尺度資訊，卻仍以 \texttt{edge\_variance\_floor} 與 \texttt{max\_edge\_weight} 所允許的權重進入加權最小平方——那是數值安全常數，不是統計論證。\texttt{simulate\_racergt\_data} 在取視窗最大值\emph{之前}先對每個 chunk 乘上連續的對數常態噪音；在其預設參數下，兩個 chunk 取整後逐格相同的機率微乎其微，實測 520 條模擬 edge 中零筆退化，真實資料 26 條中 5 筆。（把 \texttt{chunk\_noise\_sd} 設為 $0$ 則模擬器也會產生退化 edge，但那不是它的預設，也不是先前證據所使用的設定。）1.4.0 因此在 \texttt{CalibrationResult.diagnostics} 新增 \texttt{n\_zero\_dispersion\_edges}、\texttt{n\_informative\_edges} 與 \texttt{n\_scale\_groups} 三個鍵。它們\textbf{只作報告，不改變任何估計值}；退化 edge 的處理是使用者的判斷，本套件不代為決定。
...
這份資料無法檢驗 F1，卻能決定性地檢驗另一件事：模擬器描述得對不對。\texttt{SimulationSettings.chunk\_noise\_sd} 預設 $0.03$，在取最大值前對每個 chunk 每一天乘上獨立的對數常態擾動。真實資料的重疊 log ratio 離散度中位數為 $0.0188$。
...
\centering
\small
\caption{掃描 \texttt{chunk\_noise\_sd} 並與觀測到的重疊離散度 $0.018814$ 比對。每格 200 次重複。}
