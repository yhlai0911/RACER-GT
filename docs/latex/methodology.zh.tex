...
\subsubsection{一年的 chunk 對 F1 沒有檢定力，而這可以量化}

兩個方法在真實資料上幾乎不可區分：相關係數 $1.000000$，逐日相對差中位數 $0.00026$、最大 $0.00123$，在 0--100 的指數尺度上最大絕對差僅 $0.195$ 點。F1 的特徵訊號也不存在——尺度差異並未隨鏈位置成長（末三段對前三段之比 $1.03$；Spearman $\rho=-0.024$，$p=0.955$）。

但這\textbf{不構成 F1 被推翻的證據}，因為這個設計根本累積不出東西，而這一點可以直接算出來。F1 靠累積運作，鏈末端的誤差變異數是所經各接合變異數之和，零離散度的接合貢獻為零。本設計 7 個接合中僅 3 個帶資訊，有效接合的 se 中位數為 $0.002037$，鏈末端可累積的標準差因此僅 $0.0027$ log points，即 $\mathbf{0.270\%}$。反解可得：累積到 $1\%$ 需約 24 個有效接合，$5\%$ 需 574 個，$10\%$ 需 2{,}189 個。這三個數字是\emph{下界}：所用的 se 中位數取自全部 21 條有效重疊，其中包含較短的重疊；若只計 30 日步進下相鄰接合的 150 日重疊（se 約 $0.0014$），達到 $1\%$ 需約 50 個接合。換言之，此處刻意採用對 F1 較有利的估計。

\begin{remark}[F1 是真的，但在任何可行的設計中都不重要]
\label{rem:f1scale}
以真實觀測到的重疊長度與離散度計算，逐段 stitching 的尺度誤差累積要達到 $5\%$ 需要約 574 個有效接合。以 30 天步進計，那遠超過 Google Trends 服務存在的年數。\cref{sec:calexp} 的模擬之所以看得到 $4.75$ 倍的成長，是因為模擬器的 chunk 噪音高估了約 4.5 倍。

因此本文對 F1 的正確陳述是：機制為真且可被量測，但在真實資料的雜訊水準下，其累積量在任何現實可行的 GT 設計中都不具實質意義。全域圖校準值得採用的理由不是它更準，而是\cref{tab:realdata} 中那些逐段 stitching 結構上無法提供的診斷——連通性、condition number、cycle 閉合誤差。這與 \cref{rem:pattern} 的結論一致，並把它推進了一步：先前三次是「機制成立但效果二階」，本次是「機制成立且效果的上界可被計算」。
\end{remark}
