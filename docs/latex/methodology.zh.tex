...
另有第二個每日誤差可用，現已並列輸出。校準階段對每一個 pull、每一個日期，由涵蓋該日的各 chunk 之間的不一致構造出一個標準誤，並以 delta method 轉換到 level 尺度。它是**真正逐日**的，且隨水準變動：指數值小的日期相對誤差較大——這正是 0--100 整數尺度捨入所蘊含的性質。在真實資料上，該誤差與值的比值與捨入預測 $1/(\sqrt{12}\,v)$ 在各值域內吻合至 3\%--12\%。

**兩者並列而不相加。** $\hat{\Sig}$ 由殘差估計，而殘差已包含校準誤差，相加會重複計算。校準誤差提供的是一個**獨立的下界**：它來自當日 chunk 之間的不一致，與殘差共變異數毫無共用推導，因此校準誤差超過共識標準誤的日期，正是殘差式估計偏小的日期；該比例會被報告。以共識權重傳遞時假設校準誤差跨 pull 獨立——真實資料顯示同一收集日內的 chunk 是同一份被服務樣本的重新縮放，此性質是否延伸到跨 pull **未知**，故該假設以旗標隨結果輸出。

兩者的分工在於它們量的是不同的東西：校準誤差在**單一次取得之內**，由捨入與 chunk 不一致主導；共識標準誤在**跨次取得之間**，承載重複取得所揭露的抽樣變異。**只有前者不需要重複收集即可取得。**
...
\subsection{本框架適用於哪一類量測}\label{sec:appliesto-zh}

前述結果沒有一項是 Google Trends 專屬的。本框架要求的是該量測為**平台中介**（platform-mediated）：由一個分析者不控制、會改變、且不公開其作為細節的中介層所產生。四個判準決定一條序列是否屬於此類，而它們是**四個不同的問題**，不是同一個：

\begin{enumerate}
\item 該數值是**抽樣**的結果嗎——重複取得會不會不同？
\item 有沒有**請求內或樣本內的正規化**，其基準會不會移動？
\item 來源會不會**回溯改變**——同一個歷史日期在不同取得日是否有不同值？
\item 建構方法改變時，歷史會不會被**重算**？
\end{enumerate}

\begin{table}[H]
\centering
\small
\caption{本框架處理哪些序列。\textbf{僅第一列經過本文實測}；其餘各列依結構對應歸類，並非量測結果，且沒有任何一列可作為本框架的證據。}
\label{tab:appliesto-zh}
\begin{tabularx}{\textwidth}{Xcccc c}
\toprule
序列 & 抽樣 & 正規化 & 回溯改變 & 重算歷史 & 適用\\
\midrule
Google Trends（\textbf{已實測}） & 是 & 每請求 & 是 & 是 & 是\\
Baidu Index、Naver DataLab & 是 & 每請求 & 是 & 是 & 是\\
以新聞計數建構的政策不確定性指數 & 否 & 對基準 & \textbf{否}$^\dagger$ & 是 & 部分\\
相對基準期的移動性指數 & 否 & 對基準 & 是 & 是 & 部分\\
供應商的情緒與人流面板 & 面板 & 不一 & 是 & 是 & 部分\\
\midrule
\multicolumn{6}{l}{\emph{不在本框架範圍內，且值得明講}}\\
以市場變數合成的情緒指標 & 否 & 映至 $[0,100]$ & 否 & 是 & 否\\
有公開公式的波動度指數 & 否 & 否 & 否 & 是 & 否\\
\bottomrule
\end{tabularx}
\raggedright\footnotesize
$^\dagger$為實測結果而非假設，見下文。
\end{table}

**被排除的那兩列比被納入的更重要。** 一個由可觀測市場變數（動能、廣度、選擇權比率、信用利差）合成的指標並非抽樣，也不會回溯改變；它若難以使用，困難之處在於**建構方式未公開**，那是可重現性問題，需要不同的工具。有公開公式的波動度指數，任何人都能由選擇權價格重算。把本框架宣稱到它們身上會被立即反駁，而排除它們正是讓納入判準具有內容的原因。

以新聞計數建構的不確定性指數是**結構上最接近的類比**，且對應是逐項的：分母中的文章總數扮演視窗最大值的角色，語料庫收錄範圍的變動扮演 Google 改變抽樣的角色，標準化基準期扮演 baseline 的角色。本節的初稿據此把該列的「回溯改變」標為「很可能」。**該推論已被實測否定**——以發布者自身檔案的封存副本作為第二個取得日期進行檢驗。

在兩個版本共同涵蓋的 416 個月中（已排除封存版未定案的末端），**僅有一個月**的相對差異超過 $10^{-6}$，且該月是封存取得日之前第六個月，屬正常的定案落後。其餘所有差異皆低於封存檔案儲存數值所用的單精度下限，因此是**儲存格式而非修訂**。已發布的歷史值是穩定的，本框架的「回溯改變」判準並不適用。

**真正改變的是該檔案呈現的是哪一條序列。** 封存版的首個工作表以主指數命名，其第一個數值欄為三成分指標；當前版該工作表更名為 Legacy，而領頭工作表的第一個數值欄是新聞計數指標。涵蓋範圍另外向前延伸逾八十年。一位打開檔案並取用第一個數值欄的研究者，在兩個日期會得到**不同的序列**——同一發布者、同一檔名。這是可重現性問題，而且是嚴重的問題，但它是**身分的改變而非量測的改變**，因此不在本框架處理的範圍內。將它記錄於此正是「實測而非斷言」的意義所在：該類比在兩個判準上成立，卻恰好在被援引的那一個判準上失敗。

\subsection{與即時資料（real-time data）分析的關係}\label{sec:realtimedata-zh}

「取得日期是資料的一部分」這個想法對經濟學並不新。即時資料文獻 \cite{croushore2001} 已確立：一條總體序列是一連串 vintage 而非單一物件；同一季在不同 vintage 有不同值；使用最終修訂值會為「當時可知什麼」的研究帶來 look-ahead bias。

本框架是把該想法延伸到**其核心假設失效**的情形。

\begin{table}[H]
\centering
\small
\caption{為何 vintage 記錄對平台中介的序列並不充分。}
\label{tab:vintage-zh}
\begin{tabularx}{\textwidth}{lXX}
\toprule
 & 即時總體資料 & 平台中介的量測\\
\midrule
變動的性質 & 修訂：確定性更新 & 抽樣：隨機\\
是否存在最終值 & 有；修訂會收斂 & 沒有\\
正確做法 & 使用當時可得的 vintage & vintage 不足：**同一 vintage 內**兩次取得即有差異\\
不確定性來源 & 可由修訂史估計 & 需要依預先固定的設計**重複取得**\\
\bottomrule
\end{tabularx}
\end{table}

記錄取得日期（即 vintage 的作用）在此是**必要而不充分**的。由於不存在序列所收斂到的值，不確定性無法從修訂史讀出，唯一的量化途徑是依預先固定的設計重複取得。這正是本文取得設計一節的全部內容，也是本文為何是一份**重複量測 protocol** 而非一項封存慣例的原因。
...
\bibitem{croushore2001}
Croushore, D., \& Stark, T. (2001).
A real-time data set for macroeconomists.
\textit{Journal of Econometrics}, 105(1), 111--130.
