...
但這\textbf{不構成 F1 被推翻的證據}，因為這個設計根本累積不出東西，而這一點可以直接算出來。F1 靠累積運作，鏈末端的誤差變異數是所經各接合變異數之和，零離散度的接合貢獻為零。本設計 7 個接合中僅 3 個帶資訊，有效接合的 se 中位數為 $0.002037$，鏈末端可累積的標準差因此僅 $0.0027$ log points，即 $\mathbf{0.270\%}$。反解可得：累積到 $1\%$ 需約 24 個有效接合，$5\%$ 需 574 個，$10\%$ 需 2{,}189 個。
