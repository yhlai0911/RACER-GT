...
以新聞計數建構的政策不確定性指數 & 否 & 對基準 & 很可能 & 是 & 是\\
...
以新聞計數建構的不確定性指數是**結構上最接近的類比**，且對應是逐項的：分母中的文章總數扮演視窗最大值的角色，語料庫收錄範圍的變動扮演 Google 改變抽樣的角色，標準化基準期扮演 baseline 的角色。但**其已發布的歷史值是否真的會隨語料庫更新而改變，本文並未確立**。該問題可用本文所描述的程序直接檢驗——在不同日期取得同一段歷史並比對——且該檢驗不需要任何受限介面的存取權。
