...
\begin{remark}[為何仍保留共變異數調整的 consensus]
第二點是負面結果，但它界定的是適用範圍，不是對方法的否定。本模擬的跨 pull 依賴結構高度\emph{同質}：所有 pull 以固定權重 0.5 共享同一 collection-day 噪音，因此殘差共變異數矩陣近乎等相關，而等相關矩陣的最小變異權重本來就是等權。GLS 相對等權平均的理論優勢，需要依賴強度在 pulls 之間\emph{不同}才會顯現——例如部分 pull 來自同一 cache 而其餘不是。辨識這一點需要在依賴異質性維度上變動的 factorial 模擬，本版本尚未提供。

此外，simple mean 無法提供 \RACER 一併輸出的東西：spectral 與 Kish 有效 pull 數、near-duplicate 依賴結構、每日條件標準誤，以及可傳遞至下游 EIV 修正的測量不確定性。在本 DGP 下這些診斷沒有轉化為更低的 RMSE，但它們正是判斷「這批資料是否可用」所需的量，而 RMSE 本身無法回答該問題。
\end{remark}
