...
If $\epsilon_{jt}\sim\mathcal{N}(0,\sigma_j^2)$ in
$Y_{jt}=a_jX_t\exp(\epsilon_{jt})$, then
$\E[Y_{jt}]=a_jX_t\exp(\sigma_j^2/2)$. Back-transforming with
$\exp(-\hat{\ell}_j)$ alone therefore targets the median rather than the mean.
The optional correction multiplies by $\exp(-\hat{\sigma}_j^2/2)$.

\begin{remark}[Departure 2]
This correction is exact only under lognormality. It is enabled by default
because the multiplicative model motivates it, but it is a configuration flag
precisely because the normality assumption is not testable at the sample sizes
involved.
\end{remark}
...
\end{equation}
The matrix $Q$ is built as $\rho_1 I+\rho_2 D_2^\top D_2$ where $D_2$ is the
second-difference operator, so the first term penalizes departure from the daily
consensus and the second penalizes roughness in the correction path. Movement
preservation is thus imposed on the correction rather than on the level.
...
% =============================================================================
\section{Measurement-error correction}

\subsection{Moment correction}
