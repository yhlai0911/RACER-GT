...
對 `S_tilde` 做特徵分解。若 `S_tilde` 失去半正定性，**不會靜默投影回正定錐**。依 `prop:factor` (iii)，模型正確且 `Omega` 正確時 `S_tilde` 的期望值必然半正定，因此非正定代表**供給的 `Omega` 相對於觀測共變異數過大**——量測誤差被高估、模型設定錯誤、或有限樣本變異。這是關於輸入的結論，不是數值問題。
...
| 逐段 stitching，**真實資料** | 無法區分：0–100 尺度上最大差 0.195 點 |
| West (2020) anchor bank | 未比較——需要多組 query |
| Djorno et al. (2026) preprocessing | 未比較——以下游預測準確度評估 |

上表除「真實資料」那一列外，全部來自本套件自己的模擬器，而模擬器是依估計量所假設的模型抽樣的。1.4.0 首次以真實 Google Trends 下載資料（`bitcoin`、美國、日頻、2024 年、8 個 chunk）進行檢驗，得到三件事：

- **模擬器高估 chunk 噪音約 4.5 倍。** 其 `chunk_noise_sd = 0.03` 產生的重疊離散度是實際觀測值的 2.47 倍；配適值為 0.0066，而單純的整數捨入就解釋了真實重疊離散度的 94%。
- **GT 以視窗最大值正規化，所以共用峰值日的重疊 chunk 回傳完全相同的數列。** 26 條重疊中有 5 條零資訊，8 個 chunk 只構成 4 個正規化群。模擬器結構上產生不了這個現象；1.4.0 以 `n_zero_dispersion_edges` 與 `n_scale_groups` 報告它，但不改變任何估計值。
- **逐段 stitching 的誤差累積現在是定理，不是傳說。** Graph WLS 的權重就是 edge 精度，因此回復的 log scale 變異數正好是到參考 chunk 的**有效電阻**，而 spanning path 是電阻串聯。由 Rayleigh 單調性，全域圖**永不更差**，且差距可在收資料前由設計算出。本次一年設計**實測**降幅 9.1 倍，兩者終端誤差都只有 0.40%。外推到 15 年、15 日步進為 2.79% 對 0.23%——那是**預測不是量測**，來自單一關鍵字、單一 pull，論文已明列那一年資料否證了它的哪些假設。

真實資料沒有推翻比例模型，且可偵測效果已明列而非假定：45 個可檢驗三角形的 cycle 閉合誤差中位數為 0.0018 log points，而對單一 chunk 注入「每 100 天 2%」的趨勢會使最大閉合誤差加倍，故該量級以上的違反可被排除。第四項檢查——每 chunk 對重建數列的偏離——檢定力**恰好為零**，因為整體縮放在構造上就被估計出的尺度吸收；報告它只是為了指出這一點。

**只有校準階段經過這樣的檢驗。** 這份資料是單一 pull，因此共識加權、duplicate 診斷、G-theory 與 benchmark 仍然只有模擬證據。
