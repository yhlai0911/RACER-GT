...
- **逐段 stitching 的誤差累積現在是定理，不是傳說。** Graph WLS 的權重就是 edge 精度，因此回復的 log scale 變異數正好是到參考 chunk 的**有效電阻**，而 spanning path 是電阻串聯。由 Rayleigh 單調性，全域圖**永不更差**，且差距可在收資料前由設計算出。本次一年設計降幅 9.1 倍，但兩者終端誤差都只有 0.40%；15 年、15 日步進則是 2.79% 對 0.23%。
